...
// File generated at Wed 9 Jul 2014 14:04:09
 ...
 */
Eigen::Matrix<double,3,2> NE6SSM_soft_parameters::calc_beta_Tfu_one_loop(const Soft_traces& soft_traces) const
{
 ...
 */
Eigen::Matrix<double,3,2> NE6SSM_soft_parameters::calc_beta_Tfu_two_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_Tfu = twoLoop*(5.94*Power(g1,4)*Tfu + 19.665*Power(g1p,4)*Tfu +
      ...
      traceYuAdjYu*Sqr(g1p)*Tfu + 0.25*AbsSqr(Lambdax)*Sqr(g1p)*Tfu + 0.39*Sqr(
      ...
      MassBp*tracefuAdjfu*Sqr(g1p) - 30*MassBp*traceYuAdjYu*Sqr(g1p) + 39*MassB
      *Sqr(g1)*Sqr(g1p) + 39*MassBp*Sqr(g1)*Sqr(g1p) + 180*MassB*Sqr(g1)*Sqr(g2
