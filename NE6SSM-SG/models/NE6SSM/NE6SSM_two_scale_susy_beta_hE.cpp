...
// File generated at Wed 9 Jul 2014 13:58:20
 ...
 */
Eigen::Matrix<double,3,2> NE6SSM_susy_parameters::calc_beta_hE_one_loop(const Susy_traces& susy_traces) const
{
 ...
 */
Eigen::Matrix<double,3,2> NE6SSM_susy_parameters::calc_beta_hE_two_loop(const Susy_traces& susy_traces) const
{
   ...
   beta_hE = twoLoop*(hE*(18.9*Power(g1,4) + 6.825*Power(g1p,4) + 16.5*
      ...
      *fu) - 2*(hE*fu.adjoint()*fu*fu.adjoint()*fu) - 2*(hE*fu.adjoint()*fu*
      ...
      - 2*(Ye*Ye.adjoint()*Ye*Ye.adjoint()*hE));
