...
// File generated at Wed 9 Jul 2014 14:03:59
 ...
 */
Eigen::Matrix<double,3,2> NE6SSM_soft_parameters::calc_beta_ThE_one_loop(const Soft_traces& soft_traces) const
{
 ...
 */
Eigen::Matrix<double,3,2> NE6SSM_soft_parameters::calc_beta_ThE_two_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_ThE = twoLoop*(18.9*Power(g1,4)*ThE + 6.825*Power(g1p,4)*ThE +
      ...
      tracegDAdjgD*Sqr(g1p)*ThE + 0.3*tracehEAdjhE*Sqr(g1p)*ThE + 0.51*Sqr(g1)*
      ...
      *traceAdjgDTgD*Sqr(g1p) - 30*traceAdjhEThE*Sqr(g1p) + 90*MassBp*
      tracegDAdjgD*Sqr(g1p) + 30*MassBp*tracehEAdjhE*Sqr(g1p) + 51*MassB*Sqr(g1
      )*Sqr(g1p) + 51*MassBp*Sqr(g1)*Sqr(g1p) + 180*MassB*Sqr(g1)*Sqr(g2) + 180
