...
// File generated at Tue 8 Jul 2014 14:46:34
 ...
 */
Eigen::Matrix<double,3,3> NE6SSM_soft_parameters::calc_beta_TgD_one_loop(const Soft_traces& soft_traces) const
{
 ...
 */
Eigen::Matrix<double,3,3> NE6SSM_soft_parameters::calc_beta_TgD_two_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_TgD = twoLoop*(4.588888888888889*Power(g1,4)*TgD + 6.825*Power(
      ...
      0.24333333333333335*Sqr(g1)*Sqr(g1p)*TgD + Sqr(g1)*Sqr(g2)*TgD + 0.75*Sqr
      ...
      *Sqr(g1p) + 0.6*traceAdjhEThE*Sqr(g1p) - 1.8*MassBp*tracegDAdjgD*Sqr(g1p)
      - 0.6*MassBp*tracehEAdjhE*Sqr(g1p) - 0.4866666666666667*MassB*Sqr(g1)*
      Sqr(g1p) - 0.4866666666666667*MassBp*Sqr(g1)*Sqr(g1p) - 2*MassB*Sqr(g1)*
