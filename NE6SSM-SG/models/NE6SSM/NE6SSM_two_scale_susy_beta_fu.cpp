...
// File generated at Tue 8 Jul 2014 14:40:51
 ...
 */
Eigen::Matrix<double,3,2> NE6SSM_susy_parameters::calc_beta_fu_one_loop(const Susy_traces& susy_traces) const
{
 ...
 */
Eigen::Matrix<double,3,2> NE6SSM_susy_parameters::calc_beta_fu_two_loop(const Susy_traces& susy_traces) const
{
   ...
   beta_fu = twoLoop*(0.005*fu*(1188*Power(g1,4) + 3933*Power(g1p,4) +
      ...
      *hE) - 0.2*Sqr(g1p)*(fu*hE.adjoint()*hE) - 3*traceKappaAdjKappa*(fu*(
      ...
      fd.transpose()*fd.conjugate()*Lambda12);
