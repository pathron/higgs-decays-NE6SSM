...
// File generated at Fri 13 Jun 2014 15:36:40
 ...
 */
Eigen::Matrix<double,3,3> NE6SSM_soft_parameters::calc_beta_mq2_one_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_mq2 = oneOver16PiSqr*(2*mHd2*(Yd.adjoint()*Yd) + 2*mHu2*(
      ...
      0.2581988897471611*g1*Tr11*UNITMATRIX(3) + 0.31622776601683794*g1p*Tr14*
      UNITMATRIX(3) - 0.13333333333333333*AbsSqr(MassB)*Sqr(g1)*UNITMATRIX(3) -
      0.2*AbsSqr(MassBp)*Sqr(g1p)*UNITMATRIX(3) - 6*AbsSqr(MassWB)*Sqr(g2)*
      UNITMATRIX(3) - 10.666666666666666*AbsSqr(MassG)*Sqr(g3)*UNITMATRIX(3));
 ...
 */
Eigen::Matrix<double,3,3> NE6SSM_soft_parameters::calc_beta_mq2_two_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_mq2 = twoLoop*(-6*traceconjTYdTpTYd*(Yd.adjoint()*Yd) - 2*
      ...
      *Yd) + 0.8*mHd2*Sqr(g1)*(Yd.adjoint()*Yd) + 1.2*mHd2*Sqr(g1p)*(Yd.adjoint
      ...
      *TYd) - 2*traceYeAdjYe*((TYd).adjoint()*TYd) - 2*AbsSqr(Lambdax)*((TYd)
      ...
      )*((TYu).adjoint()*Yu) - 0.4*MassBp*Sqr(g1p)*((TYu).adjoint()*Yu) - 2*
      Conj(Lambdax)*TLambdax*((TYu).adjoint()*Yu) - 6*traceYuAdjYu*((TYu)
      .adjoint()*TYu) - 2*AbsSqr(Lambdax)*((TYu).adjoint()*TYu) + 1.6*Sqr(g1)*(
      (TYu).adjoint()*TYu) + 0.4*Sqr(g1p)*((TYu).adjoint()*TYu) - 3*
      ...
      ) + 0.6*Sqr(g1p)*(mq2*Yd.adjoint()*Yd) - 3*traceYuAdjYu*(mq2*Yu.adjoint()
      *Yu) - AbsSqr(Lambdax)*(mq2*Yu.adjoint()*Yu) + 0.8*Sqr(g1)*(mq2*
      Yu.adjoint()*Yu) + 0.2*Sqr(g1p)*(mq2*Yu.adjoint()*Yu) - 6*traceYdAdjYd*(
      Yd.adjoint()*md2*Yd) - 2*traceYeAdjYe*(Yd.adjoint()*md2*Yd) - 2*AbsSqr(
      Lambdax)*(Yd.adjoint()*md2*Yd) + 0.8*Sqr(g1)*(Yd.adjoint()*md2*Yd) + 1.2*
      Sqr(g1p)*(Yd.adjoint()*md2*Yd) - 3*traceYdAdjYd*(Yd.adjoint()*Yd*mq2) -
      traceYeAdjYe*(Yd.adjoint()*Yd*mq2) - AbsSqr(Lambdax)*(Yd.adjoint()*Yd*mq2
      ) + 0.4*Sqr(g1)*(Yd.adjoint()*Yd*mq2) + 0.6*Sqr(g1p)*(Yd.adjoint()*Yd*mq2
      ) - 6*traceYuAdjYu*(Yu.adjoint()*mu2*Yu) - 2*AbsSqr(Lambdax)*(Yu.adjoint(
      )*mu2*Yu) + 1.6*Sqr(g1)*(Yu.adjoint()*mu2*Yu) + 0.4*Sqr(g1p)*(Yu.adjoint(
      ...
      )*AbsSqr(MassG)*UNITMATRIX(3) + 51*Power(g2,4)*AbsSqr(MassWB)*UNITMATRIX(
      ...
      *Sqr(g1p)*Sqr(g2)*UNITMATRIX(3) + 0.3*MassBp*Conj(MassWB)*Sqr(g1p)*Sqr(g2
      ...
      Yu.adjoint()*TYu)) + (1518*MassB*Sqr(g1) - 51*(2*MassB + MassBp)*Sqr(g1p)
      + 10*(9*(2*MassB + MassWB)*Sqr(g2) + 16*(2*MassB + MassG)*Sqr(g3)))*
      UNITMATRIX(3)) + 0.0016666666666666668*Conj(MassBp)*Sqr(g1p)*(240*(6*
      MassBp*(Yd.adjoint()*Yd) - 3*(Yd.adjoint()*TYd) + 2*MassBp*(Yu.adjoint()*
      Yu) - Yu.adjoint()*TYu) + (-68*(MassB + 2*MassBp)*Sqr(g1) + 20*(9*(2*
      MassBp + MassWB)*Sqr(g2) + 16*(2*MassBp + MassG)*Sqr(g3)) + 9*MassBp*Sqr(
      g1p)*(251 + 2*Sqr(QS)))*UNITMATRIX(3)));
