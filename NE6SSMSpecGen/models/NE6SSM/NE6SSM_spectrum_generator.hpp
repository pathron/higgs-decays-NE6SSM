...
#ifndef NE6SSM_SPECTRUM_GENERATOR_H
#define NE6SSM_SPECTRUM_GENERATOR_H
...
template <class T>
class NE6SSM_spectrum_generator {
public:
   NE6SSM_spectrum_generator()
      : solver(), model()
   ...
   const NE6SSM<T>& get_model() const { return model; }
   ...
   void run(const QedQcd& oneset, const NE6SSM_input_parameters& input);
   void write_running_couplings(const std::string& filename = "NE6SSM_rge_running.dat") const;
   void write_spectrum(const std::string& filename = "NE6SSM_spectrum.dat") const;

private:
   RGFlow<T> solver;
   NE6SSM<T> model;
   NE6SSM_susy_scale_constraint<T> susy_scale_constraint;
   NE6SSM_low_scale_constraint<T>  low_scale_constraint;
 ...
 */
template <class T>
void NE6SSM_spectrum_generator<T>::run(const QedQcd& oneset,
                                const NE6SSM_input_parameters& input)
{
   ...
   NE6SSM_convergence_tester<T> convergence_tester(&model, precision_goal);
   if (max_iterations > 0)
      convergence_tester.set_max_iterations(max_iterations);

   NE6SSM_initial_guesser<T> initial_guesser(&model, input, oneset,
                                                  low_scale_constraint,
                                                  susy_scale_constraint);
 ...
 */
template <class T>
void NE6SSM_spectrum_generator<T>::write_running_couplings(const std::string& filename) const
{
   NE6SSM<T> tmp_model(model);
   tmp_model.run_to(low_scale);

   NE6SSM_parameter_getter parameter_getter;
   Coupling_monitor<NE6SSM<T>, NE6SSM_parameter_getter>
 ...
 */
template <class T>
void NE6SSM_spectrum_generator<T>::write_spectrum(const std::string& filename) const
{
   NE6SSM_spectrum_plotter plotter;
