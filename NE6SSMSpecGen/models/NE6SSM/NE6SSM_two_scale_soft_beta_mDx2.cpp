 ...
 */
Eigen::Matrix<double,3,3> NE6SSM_soft_parameters::calc_beta_mDx2_one_loop(const Soft_traces& soft_traces) const
{
 ...
 */
Eigen::Matrix<double,3,3> NE6SSM_soft_parameters::calc_beta_mDx2_two_loop(const Soft_traces& soft_traces) const
{
   ...
   beta_mDx2 = twoLoop*(-6*traceconjTKappaTpTKappa*(Kappa.conjugate()*(
      Kappa).transpose()) - 12*ms2*traceKappaAdjKappa*(Kappa.conjugate()*(Kappa
      ).transpose()) - 6*traceKappaAdjKappaconjmDx2*(Kappa.conjugate()*(Kappa)
      .transpose()) - 6*traceKappaconjmDxbar2AdjKappa*(Kappa.conjugate()*(Kappa
      ).transpose()) - 4*mHd2*AbsSqr(Lambdax)*(Kappa.conjugate()*(Kappa)
      .transpose()) - 4*mHu2*AbsSqr(Lambdax)*(Kappa.conjugate()*(Kappa)
      .transpose()) - 8*ms2*AbsSqr(Lambdax)*(Kappa.conjugate()*(Kappa)
      .transpose()) - 2*mphi2*AbsSqr(Sigmax)*(Kappa.conjugate()*(Kappa)
      .transpose()) - 4*ms2*AbsSqr(Sigmax)*(Kappa.conjugate()*(Kappa).transpose
      ()) - 2*msbar2*AbsSqr(Sigmax)*(Kappa.conjugate()*(Kappa).transpose()) - 4
      *AbsSqr(TLambdax)*(Kappa.conjugate()*(Kappa).transpose()) - 2*AbsSqr(
      TSigmax)*(Kappa.conjugate()*(Kappa).transpose()) + 0.5*ms2*Sqr(g1p)*(
      ...
      AbsSqr(Lambdax)*(Kappa.conjugate()*mDxbar2*(Kappa).transpose()) - 2*
      ...
      Kappa.conjugate()*(Kappa).transpose()*mDx2) - 4*ms2*(Kappa.conjugate()*(
      ...
      UNITMATRIX(3) + 0.017777777777777778*Conj(MassB)*Sqr(g1)*(768*MassB*Sqr(
      g1) - 21*(2*MassB + MassBp)*Sqr(g1p) + 80*(2*MassB + MassG)*Sqr(g3))*
      ...
      )*(TKappa).transpose()) + (-56*(MassB + 2*MassBp)*Sqr(g1) + 320*(2*MassBp
