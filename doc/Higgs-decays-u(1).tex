...
\label{hd10}
\ee
The equations for the extrema of the Higgs boson potential in Eq.~(\ref{hd9}) in the directions of Eq.~(\ref{hd10}) read:
\be
\ba{rcl}
...
\label{hd11}
\ee
where $\bar{g}=\sqrt{g_2^2+g'^2}$. Instead of specifying
$v_1$, $v_2$, $s_1$ and $s_2$, it is more convenient to use $\tan\beta=v_2/v_1$,
...
generation differs substantially. Let the $Z'$ and $Z$ states be the gauge bosons associated with $U(1)_{N}$ and
SM--like $Z$ boson respectively. Then the $Z-Z'$ mass-squared matrix is given by
\be
M^2_{ZZ'}=\left(
\ba{cc}
...
$$
The fields $S$ and $\overline{S}$ must acquire large VEVs,i.e.,
 $s_1\simeq s_2 \gg 1\,\mbox{TeV}$, to ensure
that the extra $U(1)_{N}$ gauge boson is sufficiently heavy. Then the mass of the lightest neutral gauge boson $Z_1$ is very close
...
\label{hd19}
\ee
In Eqs.~(\ref{hd19}) $\tilde{\Delta}_{ij}$ correspond to the contribution of loop corrections. Since in the models under consideration $s$ must be much larger
than $v$, it follows that $\gamma$ goes to zero. Moreover, since in 
phenomenologically acceptable SUSY models the supersymmetry breaking scale also tends
to be considerably larger than $v$, the mixing between $P_1$ and pseudoscalar states is somewhat suppressed. The pseudoscalar states
are mainly made up of linear superpositions of the imaginary
parts of the SM singlet fields $S$, $\overline{S}$ and $\phi$, i.e. $P_2$ and $P_3$.
 In other words, the off--diagonal entries
$\tilde{M}_{12}^2,\, \tilde{M}_{13}^2\ll \tilde{M}_{11}^2$ allow the 
diagonalization of the mass matrix (\ref{hd18})--(\ref{hd19}) to be
performed analytically.
In particular, the mass of the CP--odd Higgs eigenstate that is predominantly $P_1$, 
is set by $\tilde{M}_{11}^2$. As a consequence
this CP--odd state and the charged physical Higgs states are expected to be
...
%% \includegraphics[width=0.5\textwidth]{logKappa_vs_Akappa_vs_logBR_alt.eps}
%% \caption{Left panel: colour contours of the branching ratio for
%%   $h_1\rightarrow A_1 A_1$ in the $\kappa$-$A_\kappa$ plane for small
%%   $\kappa$. Right Panel: as on the left panel but with a wider range
%%   of $\kappa$.  An adapted version of FlexibleSUSY
%%   generated code was used to calculate mass spectrum and the branching
%%   ratio.  All other parameters are fixed to the values BM1.  For each
%%   value of $\kappa$ there is a lower limit on $A_\kappa$ where the the
%%   BR is zero because the pseudoscalar is too heavy and an upper limit
%%   above which there is a pseudoscalar tachyon.  }
%% \end{figure}

\begin{figure}[h!]
\begin{center}
\includegraphics[width=0.8\textwidth]{Kappa_vs_Akappa_vs_BR_logplot.eps}
...
\bibitem{Allanach:2001kg} 
  ...
  %804 citations counted in INSPIRE as of 19 Aug 2014
...
\bibitem{Allanach:2013kza} 
  ...
  %4 citations counted in INSPIRE as of 19 Aug 2014
